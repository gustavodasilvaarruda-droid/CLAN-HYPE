...
def create_expansion_blueprint(supabase, login_required, safe_table, is_admin, registrar_log,
    ...
    def admin_dashboard():
        ...
        stats = {
            ...
            'breed_producao': sum(x.get('status') == 'em_producao' for x in breeds),
        ...
        logs = sorted(safe_table('logs_admin'), key=lambda x: x.get('created_at') or '', reverse=True)[:15]
        return render_template('admin_dashboard_geral.html', stats=stats, logs=logs)
    ...
    def team_builder():
        ...
        return render_template('team_builder.html', meus_times=meus, times_publicos=publicos, formatos=formatos, builds_pessoais=builds_pessoais, prefill_pokemon=prefill_pokemon)
