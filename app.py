...
def parse_valor_moeda(valor):
    """Aceita 500k, 1.5m, 1,5m ou números inteiros."""
    t=str(valor or '0').strip().lower().replace(' ','').replace('.','').replace(',','.')
    mult=1
...
def _pokeapi_species_rule(pokemon_id):
    """Consulta a espécie para validar Egg Group. Cache evita chamadas repetidas."""
    try:
        ...
        breedavel = "no-eggs" not in egg_groups
...
def regra_breed_pokemon(pokemon_id, pokemon_nome):
    ...
    bloqueio = next(
        (x for x in bloqueados if (x.get("pokemon") or "").strip().lower() == nome),
        None
    )
    if bloqueio:
        return {
    ...
    if externa and not externa.get("breedavel"):
        return {
            "breedavel": False,
            "so_com_ditto": False,
            "breed_especial": False,
            "motivo": "Este Pokémon pertence ao Egg Group No Eggs Discovered e não pode breedar.",
            "egg_groups": externa.get("egg_groups") or []
        }

    so_com_ditto = bool(
        meta.get("usa_ditto_padrao")
        or (externa and externa.get("so_com_ditto"))
    )
    return {
        "breedavel": True,
        "so_com_ditto": so_com_ditto,
        "breed_especial": so_com_ditto,
        "motivo": None,
        "egg_groups": (externa or {}).get("egg_groups", [])
    }
...
def breed():
    ...
    if request.method == 'POST':
        ...
        ).eq('usuario_email', email).in_('status', ['pendente','em_andamento']).execute().data or []
        duplicado = any(
            ...
            for pedido in res_verificacao.data:
                ...
                try:
                    if data_referencia and agora - data_referencia > timedelta(days=3) and not pedido.get('prazo_notificado'):
                        # O prazo nasce quando o Breeder assume. Não apagamos o responsável:
...
def cancelar_breed(pedido_id):
    ...
    try:
        ...
        supabase.table('pedidos_breed').update({'status':'cancelado','cancelado_em':agora_iso(),'cancelado_por':email,'motivo_cancelamento':motivo or None}).eq('id',pedido_id).execute()
        registrar_historico('breed',pedido_id,p.get('status'),'cancelado',motivo)
        registrar_log('cancelar','breed','pedido_breed',pedido_id,{'motivo':motivo})
    ...
    except Exception as e: flash(f'Erro ao cancelar pedido: {e}', 'erro')
    return redirect(url_for('breed'))
...
@login_required
def lojas():
    itens = sorted(_safe_table('lojas_reino'), key=lambda x: int(x.get('posicao') or 99))
...
def loja_detalhe(loja_id):
    ...
    return render_template('loja_detalhe.html', loja=loja, produtos=produtos, pedidos=pedidos, pode_gerenciar=admin or dono)
...
def painel_reino():
    ...
    return render_template('reino.html', breeds=breeds, construcoes=construcoes, lojas_pedidos=lojas_pedidos, eventos=eventos, atividades=atividades)
